...
class Processor:
    ...
    def __init__(self, texfile: str, processor: str, encoding: str):
        ...
        # copy the attributes
        (self._texfile, self._processor, self._encoding) = (texfile, processor, encoding)
    ...
    def process_warnings(self):
        ...
        if not utils.check_file_exists(log_filename):
            print(f" [Warning]: no file found with the name {log_filename}")
            return
        ...
        log_text = pathlib.Path(log_filename).read_text(encoding=self._encoding, errors="replace")
